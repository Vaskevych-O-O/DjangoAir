...
from air.models import Flight, TicketStatusChoices, AirlineUser
...
def test_register_view_post_existing_email(client, django_user_model):
    existing_user = django_user_model.objects.create_user(email='exist@example.com', password='pass', username='testuser')
    url = reverse('register')
    data = {
        'first_name': 'test',
        'last_name': 'test',
...
def test_logout_view_post(client, django_user_model):
    user = django_user_model.objects.create_user(email='logout@example.com', password='pass', username='testuser_logout')
    client.force_login(user)
    url = reverse('logout')
...
def test_create_checkout_session_post_success(mocker, client, django_user_model):
    user = django_user_model.objects.create_user(email='checkout@example.com', password='pass', username='user_checkout1')
    client.force_login(user)
...
def test_create_checkout_session_post_exception(mocker, client, django_user_model):
    user = django_user_model.objects.create_user(email='checkout2@example.com', password='pass', username='user_checkout2')
    client.force_login(user)
    url = reverse('create_checkout_session')
